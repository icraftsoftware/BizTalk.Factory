...
namespace Be.Stateless.BizTalk.Logging.Appender
{
	/// <summary>
	/// This appender internally uses a <see cref="TraceProvider"/> to forward the logging events.
	/// </summary>
	/// <remarks>
	/// The <see cref="TraceProvider"/> is instantiated using the custom component GUID
	/// (6A223DEA-F806-4523-BAD0-312DCC4F63F9). In order to capture the logging events using Event Tracing for Windows
	/// (ETW), some configuration (beyond the classical log4net configuration) is needed. These configuration tasks are
	/// executed using the <c>StartTrace.cmd</c> and <c>StopTrace.cmd</c> scripts that can be found in utils\Tracing.
	/// There is a mapping between the <see cref="log4net.Core.Level"/> of the logging event and the <see
	/// cref="TraceLevel"/> used to output the message:
	/// <list type="bullet">
