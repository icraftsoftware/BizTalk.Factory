...
namespace Be.Stateless.BizTalk.ClaimStore.Configuration
{
	...
	public class ClaimStoreConfigurationSectionFixture
	{
		...
		{
			var claimStoreConfigurationSection = ((ClaimStoreConfigurationSection) ConfigurationManager.GetSection(
				"be.stateless.test/biztalk/claimStore"));
		...
		{
			var claimStoreConfigurationSection = (ClaimStoreConfigurationSection) ConfigurationManager.GetSection(
				"be.stateless.test/biztalk/claimStoreWithoutPollingInterval");
		...
		public void InexistentCheckOutDirectory()
		{
			Assert.That(
				...
				Throws.TypeOf<ConfigurationErrorsException>()
					.With.Message.StartsWith(@"The value for the property 'checkOutDirectory' is not valid. The error is: Could not find a part of the path 'c:\some-inexistent-folder'."));
		}
