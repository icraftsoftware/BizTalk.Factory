...
namespace Be.Stateless.BizTalk.Policies.Receive.Claim
{
	[TestFixture]
	public class CheckOutResolverFixture : PolicyFixture<CheckOutResolver>
	{
		[Test]
		...
		{
			ExecutePolicy();
			Facts.Verify(Context.Property(TrackingProperties.ProcessName).WithValue(Factory.Processes.Claim.CheckOut).HasBeenWritten());
		}
	}
}
