...
{
	[TestFixture]
	public class ProcessResolverFixture : PolicyFixture<ProcessResolver>
	{
		[Test]
		...
		{
			ExecutePolicy();
			Facts.Verify(Context.Property(TrackingProperties.ProcessName).WithValue(Factory.ServiceArea.Claim.Processes.Check).HasBeenWritten());
		}
	}
}
