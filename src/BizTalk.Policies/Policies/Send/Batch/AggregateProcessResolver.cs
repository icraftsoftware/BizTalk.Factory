...
{
	public class AggregateProcessResolver : RuleSet
	{
		public AggregateProcessResolver()
		{
			Rules.Add(
				Rule("BatchAggregateProcessName")
					.If(() => Context.Read(TrackingProperties.ProcessName).IsNullOrEmpty())
					.Then(() => Context.Write(TrackingProperties.ProcessName, Factory.Services.Batch.Processes.Aggregate))
				);
		}
	}
}
