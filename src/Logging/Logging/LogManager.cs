...
namespace Be.Stateless.Logging
{
	...
	/// <remarks>
	/// <para>
	/// This class is an enhanced (plagiarism) replacement of <see cref="log4net.LogManager"/>; it offers
	/// factory methods that return <see cref="ILog"/> references instead of <see
	/// cref="log4net.ILog"/> ones. <see cref="ILog"/> provides all of the levels
	/// defined by <see cref="log4net.Core.Level"/>. The complete list of levels by order of preference is:
	/// OFF, EMERGENCY, FATAL, ALERT, CRITICAL, SEVERE, ERROR, WARN, NOTICE, INFO, DEBUG, FINE, TRACE, FINER,
	/// VERBOSE, FINEST, ALL.
	/// </para>
	/// <para>
	/// This class has static methods that are used by a client to request a logger instance. The <see
	/// cref="GetLogger(string)"/> method is used to retrieve a logger.
	/// </para>
	/// <para>
	...
	/// 
	/// log.Info("Application Start"); log.Debug("This is a debug message");
	/// 
	...
	public static class LogManager
	{
		...
		/// <remarks>
		/// <para>
		/// If the named logger exists (in the default repository) then it returns a reference to the logger,
		...
		/// <remarks>
		/// <para>
		/// If the named logger exists (in the specified repository) then it returns a reference to the
		/// logger, otherwise it returns
		/// <c>null</c>.
		/// </para>
		/// </remarks>
		...
		/// <param name="name">The fully qualified logger name to look for.</param>
		/// <returns>
		/// The logger found, or <c>null</c> if the logger doesn't exist in the specified repository.
		/// </returns>
		...
		/// <remarks>
		/// <para>
		/// If the named logger exists (in the repository for the specified assembly) then it returns a
		/// reference to the logger, otherwise it returns
		/// <c>null</c>.
		/// </para>
		/// </remarks>
		...
		/// <param name="name">The fully qualified logger name to look for.</param>
		/// <returns>
		/// The logger, or <c>null</c> if the logger doesn't exist in the specified assembly's repository.
		/// </returns>
		...
		/// <remarks>
		/// <para>
		/// Retrieves a logger named as the <paramref name="name"/> parameter. If the named logger already
		/// exists, then the existing instance will be returned. Otherwise, a new instance is created.
		/// </para>
		/// <para>By default, loggers do not have a set level but inherit it from the hierarchy. This is one
		/// of the central features of log4net.
		/// </para>
		/// </remarks>
		...
		/// <remarks>
		/// <para>
		/// Retrieve a logger named as the <paramref name="name"/> parameter. If the named logger already
		/// exists, then the existing instance will be returned. Otherwise, a new instance is created.
		/// </para>
		/// <para>
		/// By default, loggers do not have a set level but inherit it from the hierarchy. This is one of the
		/// central features of log4net.
		/// </para>
		/// </remarks>
		/// <param name="repository">The repository to lookup in.</param>
		/// <param name="name">The name of the logger to retrieve.</param>
		...
		/// </remarks>
		/// <param name="type">The full name of <paramref name="type"/> will be used as the name of the
		/// logger to retrieve.</param>
		/// <returns>The logger with the name specified.</returns>
		...
		/// and has a no arg constructor. An instance of this type will be created to act as the <see
		/// cref="ILoggerRepository"/> for the repository specified.</param>
		/// <returns>The <see cref="ILoggerRepository"/> created for the repository.</returns>
		/// <remarks>
		/// <para>
		/// The <see cref="ILoggerRepository"/> created will be associated with the repository specified such
		/// that a call to <see cref="GetRepository()"/> will return the same repository instance.
		/// </para>
		/// </remarks>
		...
		/// </para>
		/// <para>
		/// The <paramref name="repository"/> name must be unique. Repositories cannot be redefined. An <see
		/// cref="Exception"/> will be thrown if the repository already exists.
		/// </para>
		/// </remarks>
		/// <param name="repository">The name of the repository, this must be unique amongst
		/// repositories.</param>
		/// <returns>The <see cref="ILoggerRepository"/> created for the repository.</returns>
		...
		/// </para>
		/// </remarks>
		/// <param name="repository">The name of the repository, this must be unique to the
		/// repository.</param>
		/// <param name="repositoryType">A <see cref="Type"/> that implements <see cref="ILoggerRepository"/>
		/// and has a no arg constructor. An instance of this type will be created to act as the <see
		/// cref="ILoggerRepository"/> for the repository specified.</param>
		...
		/// <para>
		/// The <see cref="ILoggerRepository"/> created will be associated with the repository specified such
		/// that a call to <see cref="GetRepository(Assembly)"/> with the same assembly specified will return
		...
		/// <remarks>
		/// <para>
		/// Calling this method will <b>safely</b> close and remove all appenders in all the loggers
		/// including root contained in all the default repositories.
		/// </para>
		/// <para>
		/// Some appenders need to be closed before the application exits. Otherwise, pending logging events
		/// might be lost.
		/// </para>
		/// <para>The <c>shutdown</c> method is careful to close nested appenders before closing regular
		/// appenders. This is allows configurations where a regular appender is attached to a logger and
		...
		/// <para>
		/// Calling this method will <b>safely</b> close and remove all appenders in all the loggers
		/// including root contained in the default repository.
		/// </para>
		/// <para>Some appenders need to be closed before the application exists. Otherwise, pending logging
		...
		/// <para>
		/// Calling this method will <b>safely</b> close and remove all appenders in all the loggers
		/// including root contained in the
		/// <paramref name="repository"/> specified.
		/// </para>
		/// <para>
		/// Some appenders need to be closed before the application exists. Otherwise, pending logging events
		/// might be lost.
		/// </para>
		/// <para>The <c>shutdown</c> method is careful to close nested appenders before closing regular
		/// appenders. This is allows configurations where a regular appender is attached to a logger and
		...
		/// <para>
		/// Calling this method will <b>safely</b> close and remove all appenders in all the loggers
		/// including root contained in the repository. The repository is looked up using the <paramref
		/// name="repositoryAssembly"/> specified.
		/// </para>
		/// <para>
		/// Some appenders need to be closed before the application exists. Otherwise, pending logging events
		/// might be lost.
		/// </para>
		/// <para>
		/// The <c>shutdown</c> method is careful to close nested appenders before closing regular appenders.
		/// This is allows configurations where a regular appender is attached to a logger and again to a
		/// nested appender.
		/// </para>
		/// </remarks>
		/// <param name="repositoryAssembly">The assembly to use to lookup the repository.</param>
		...
		/// <remarks>
		/// <para>
		/// Resets all values contained in the repository instance to their defaults. This removes all
		/// appenders from all loggers, sets the level of all non-root loggers to <c>null</c>, sets their
		/// additivity flag to <c>true</c> and sets the level of the root logger to <see
		/// cref="Level.Debug"/>. Moreover, message disabling is set to its default "off" value.
		/// </para>
		/// </remarks>
		...
		/// <remarks>
		/// <para>
		/// Reset all values contained in the repository instance to their defaults. This removes all
		/// appenders from all loggers, sets the level of all non-root loggers to <c>null</c>, sets their
		/// additivity flag to <c>true</c> and sets the level of the root logger to <see
		/// cref="Level.Debug"/>. Moreover, message disabling is set to its default "off" value.
		/// </para>
		...
		/// <remarks>
		/// <para>
		/// Gets the <see cref="ILoggerRepository"/> for the repository specified by the callers assembly
		...
		/// <remarks>
		/// <para>
		/// Gets the <see cref="ILoggerRepository"/> for the repository specified by the <paramref
		...
		/// <para>
		/// Gets the <see cref="ILoggerRepository"/> for the repository specified by the <paramref
		/// name="repositoryAssembly"/> argument.
		/// </para>
		/// </remarks>
		/// <param name="repositoryAssembly">The assembly to use to lookup the repository.</param>
		...
		{
			return (ILog) _wrapperMap.GetWrapper(logger);
		}
		...
		/// <summary>
		/// Create the <see cref="ILoggerWrapper"/> objects used by this manager.
		/// </summary>
