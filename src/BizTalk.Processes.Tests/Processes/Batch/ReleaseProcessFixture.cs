	...
	public class ReleaseProcessFixture : ProcessFixture
	{
		...
		public void ControlReleaseAllEnvelopesAndAllPartitions()
		{
			...
			var process = TrackingRepository.SingleProcess(
				p => p.Name == Factory.Processes.Batch.Release
		...
		public void TrackBatchReleasedOnControlMessage()
		{
			...
			Assert.That(addPartMessage1.Processes.SingleOrDefault(p => p.Name == Factory.Processes.Batch.Aggregate), Is.Not.Null);
			Assert.That(addPartMessage1.Processes.SingleOrDefault(p => p.Name == Factory.Processes.Batch.Release), Is.Not.Null);
			...
			Assert.That(releaseProcessBatchMessagingStep.Processes.SingleOrDefault(p => p.Name == Factory.Processes.Batch.Release), Is.Not.Null);
			Assert.That(releaseProcessBatchMessagingStep.Processes.SingleOrDefault(p => p.Name == Factory.DefaultProcesses.Unidentified), Is.Not.Null);
		...
		public void TrackBatchReleasedOnPollingWhenNoMessagingStepActivityIds()
		{
			...
			Assert.That(releaseProcessBatchMessagingStep.Processes.Count(p => p.Name == Factory.Processes.Batch.Release && p.BeginTime > StartTime), Is.EqualTo(0));
