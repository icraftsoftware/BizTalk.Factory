...
{
	public class ReleaseProcessResolver : RuleSet
	{
		public ReleaseProcessResolver()
		{
			Rules.Add(
				Rule("BatchReleaseProcessName")
					.If(() => Context.Read(TrackingProperties.ProcessName).IsNullOrEmpty())
					.Then(() => Context.Write(TrackingProperties.ProcessName, Factory.Areas.Batch.Processes.Release))
				);
		}
	}
}
